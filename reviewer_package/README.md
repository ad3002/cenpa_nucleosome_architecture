...
2. **Reference-Free Physical Caliper & MAPQ Invariance (Packages B & C):**  
   - **Physical Caliper:** Direct measurement of physical insert length from adapter-bounded raw FASTQ read overlaps without reference alignment confirms the **133-bp mode** (88.18% in [110, 140] bp) with **98.40%** exact base concordance ($R^2 = 0.999$, median difference = 0.0 bp) with BWA-MEM BAM alignment (**Fig. 5A–C**).
...
6. **Cross-Lineage Biological Replication (Package G):**  
   ...
   - **HG002 T2T Diploid (`SRR15395857`):** Phased diploid centromeres exhibit mononucleosome mode at **120–125 bp** and canonical 150-bp depletion to **0.435%**.
   - **RPE-1 Non-Transformed Diploid (`SRR9201843`):** Canonical 150-bp octamers depleted to **0.608%** (<1% across all cohorts).
   - **RPE-1 CENP-B Architectural Comparator (`SRR9201844`):** Direct sequence-specific binding protects compact sub-nucleosomal footprints (~45–65 bp) centered on the 17-bp motif, in contrast to histone variant nucleosome wrapping.
